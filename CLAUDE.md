...
這是一個**教學型專案**：目標不只是寫出可運作的問卷平台，而是照著 `LEARNING.md` 的課綱逐章建立後端能力（NestJS 11 + Prisma 7 + PostgreSQL/Neon，前端另有獨立的 **Nuxt 4** 專案）。**章數修訂過四次，目前是 Ch0–Ch18 —— 以進度表為準，不要記數字。**

**Ch13 起前後端並排放在 `Desktop/train/survey/` 底下**（`survey-backend/` 與 `survey-frontend/`，**兩個獨立的 GitHub repo**，`survey/` 本身不是 repo），Claude Code 的 session 開在 `survey/` 那一層 —— 因為「跑後端 → 讀 `/docs-json` → 在前端產型別」本來就橫跨兩個 repo。

**學習紀錄全部留在後端**：`LEARNING.md` 與 `docs/chapters/ch00`～`ch18` 都在 `survey-backend/`，不搬。理由就是下一條那句「`LEARNING.md` 是唯一的進度與範圍來源」—— 拆成兩份，「現在到哪一章」就要看兩個地方。前端 repo 只放程式碼與它自己的 `CLAUDE.md`（Vue / Nuxt 慣例），共用的教學慣例（`[教學]` 註解、閱讀動線、開工指引格式）指回這一份。

⚠️ **兩個 repo 的 commit 不能混** —— Render 接的是 `survey-backend` 的 `main`，推錯地方會觸發不該有的部署。在 `survey/` 那一層沒有 repo，每個 git 指令都要先進到子資料夾。

因此：

- **`LEARNING.md` 是唯一的進度與範圍來源。** 動手前先確認目前在哪一章、該章的驗收標準是什麼；不要提前實作後面章節的功能。它**只放進度表與索引**，不放章節內容。
- 每章完成後：在 `docs/chapters/chNN-<主題>.md` 補上該章的「核心概念 / 決策取捨 / 踩到的坑 / 作業」，然後更新 `LEARNING.md` 的進度表狀態與章節索引。**不要把章節內容寫回 `LEARNING.md`** —— 十幾章的內容會把它撐爆。
...
- **教練模式只適用後端**（Ch0–Ch12，以及階段三回頭改後端的部分）：使用者要自己寫程式碼再 review。除非明確要求「幫我寫」，否則優先解釋概念與取捨，而非直接產生整段實作。
- **前端（`survey-frontend`）由 AI 直接寫，不走教練模式。** 使用者本身是前端工程師，那部分沒有學習目標 —— review 的重點只在「它有沒有正確反映後端契約」，不在實作細節。這是 2026-09-03 第三次課綱修訂的決定，理由見 `LEARNING.md` 的「課綱修訂紀錄」。
  - 連帶的兩件事：階段三每一章的**驗收標準是後端的交付物**（見進度表）；`docs/chapters/ch13`–`ch18` 的「核心概念 / 決策取捨 / 踩到的坑」**只寫後端相關的**，前端的實作細節不進章節文件。
...
- **收尾要有終點線。** 實作階段有 e2e 綠燈當驗收，收尾沒有，所以會一直「再掃一次註解」「再補一句」—— Ch7 的收尾動了 4 個 commit，其中 `40e9d52` 回頭改的 `src/swagger.ts`，`4ca4b58` 已經改過同一支。三條規矩：
