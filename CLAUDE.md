...
- **`LEARNING.md` 是唯一的進度與範圍來源。** 動手前先確認目前在哪一章、該章的驗收標準是什麼；不要提前實作後面章節的功能。
- 每章完成後，`LEARNING.md` 需補上該章的「核心概念 / 決策取捨 / 踩到的坑 / 作業」段落，並更新進度表狀態。
- 程式碼註解偏向「解釋為什麼這樣選」而非「這行在做什麼」——沿用既有風格（見 `src/prisma/prisma.module.ts`、`src/health/health.controller.ts`）。
