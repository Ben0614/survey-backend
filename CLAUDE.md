...
這是一個**教學型專案**：目標不只是寫出可運作的問卷平台，而是照著 `LEARNING.md` 的 18 章課綱逐章建立後端能力（NestJS 11 + Prisma 7 + PostgreSQL/Neon，前端另有獨立的 **Nuxt 4** 專案）。
...
**學習紀錄全部留在後端**：`LEARNING.md` 與 `docs/chapters/ch00`～`ch17` 都在 `survey-backend/`，不搬。理由就是下一條那句「`LEARNING.md` 是唯一的進度與範圍來源」—— 拆成兩份，「現在到哪一章」就要看兩個地方。前端 repo 只放程式碼與它自己的 `CLAUDE.md`（Vue / Nuxt 慣例），共用的教學慣例（`[教學]` 註解、閱讀動線、開工指引格式）指回這一份。
...
- **前端（`survey-frontend`）由 AI 直接寫，不走教練模式。** 使用者本身是前端工程師，那部分沒有學習目標 —— review 的重點只在「它有沒有正確反映後端契約」，不在實作細節。這是 2026-09-03 第三次課綱修訂的決定，理由見 `LEARNING.md` 的「課綱修訂紀錄」。
  - 連帶的兩件事：階段三每一章的**驗收標準是後端的交付物**（見進度表）；`docs/chapters/ch13`–`ch17` 的「核心概念 / 決策取捨 / 踩到的坑」**只寫後端相關的**，前端的實作細節不進章節文件。
