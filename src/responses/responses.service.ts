...
//
// **Ch17 在這支 service 加了兩件事，而它們正好是同一個主題的兩面：**
//   輪 ④  create 多了兩道檢查（整份必答、單選答案要在選項裡）——
//         **DTO 看不到「這份問卷有幾題」，跨表的規則一律是 service 的工作**
//   輪 ⑤  summarize（統計，刻意不分頁）與 findAll 帶上 answers（消掉前端的 N+1）——
//         **同一份資料的兩種檢視，一種需要分頁、一種完全不能用分頁**
//
