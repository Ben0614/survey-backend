...
沒有它，前端硬送 `{ title: 'x', status: 'PUBLISHED' }`，那個 `status` 就有機會被塞進 `prisma.create()` —— 等於**讓呼叫端自己決定問卷發不發布**。而「發布」本該是一個有檢查的獨立動作（發布後才能被填答），不是建立時順手帶的參數。這種漏洞有名字：**mass assignment**。
...
套用到 cascade 那條測試上就是：拿掉 `schema.prisma` 裡 `Question` 的 `onDelete: Cascade`、跑 `migrate dev`，它必須變紅。**不會變紅的測試，不管寫得多漂亮，都只是佔位子。**（這正是作業第 3 題 —— 而它變紅的位置會出乎你意料。）

具體踩到的四種「假綠」樣態記在下方「踩到的坑」。**其中第 3 種還有一個更麻煩的性質：它會不會現形，取決於它旁邊那幾行寫了什麼。**
...
> **Ch6 補記（2026-08-22）：重看過了，結論是維持現狀。** 判準是「這支 service 除了 HTTP 之外還有沒有第二個呼叫者」—— 這個專案沒有，所以拆出 domain error 買到的好處是假想的。完整取捨見 [`ch06`](ch06-統一錯誤處理與回應格式.md) 的「不做 domain error」。**決定不改也是一個決定，理由已經寫進 `surveys.service.ts` 檔頭。**
...
> **Ch6 補記（2026-08-22）：比較過了，仍然選「先擋」。** 決定性的差別是**訊息品質** —— 先擋才給得出「問卷不存在」，`catch P2025` 只知道「某一筆不見了」，說不出是哪一種資源。Ch6 讓 filter 也認得 `P2025`，但定位是**縱深防禦**（漏寫一處 `assertExists` 時前端拿到 404 而不是 500），不是主要防線。
...
`.resolves.toBe()` **回傳一個 Promise**。沒有 `await`，這一行只是發動了一個非同步斷言就往下走。

**它會不會被抓到，取決於它放在哪一行**（實測，見作業第 4 題）：後面還有被 `await` 的斷言時，`it` 多活了一小段時間，Jest 30 追蹤到那個 rejection 就會判定失敗；但如果它是 `it` 的**最後一行**，測試立刻結束，錯誤直接消失 —— 綠燈。

「有時候會抓到、有時候不會」比「永遠抓不到」更危險：今天紅的東西，明天重構換個位置就默默變綠了。
...
> **第 1、2、4 題的結果是實際跑出來的**（貼的是真實輸出）。
> **第 3 題是從 Prisma 官方文件推導的** —— 沒有實跑，因為 `migrate dev` 會在版控的
> `prisma/migrations/` 裡留下兩個只為了做實驗的 migration。**你自己跑的時候請以實際輸出為準**，
> 若跟這裡寫的不一樣，相信你的終端機、然後回來改這份文件。
...
**第 3 題 —— 應該紅在 `.expect(200)`，不是紅在 `count()`。回的是 500。**（推導，未實跑）

如果你猜「`question.count()` 會是 1」，那是很自然但錯誤的推論 —— 它假設了「拿掉 `onDelete` = 什麼都不做，題目就留在那裡」。

實際上 **Prisma 對沒寫 `onDelete` 的關聯有預設值，而且預設值不是「不做事」**（下表出自 Prisma 官方文件的 Referential actions 章節）：
...
**第 4 題 —— 看它放在哪一行。這題的答案分兩種，而這正是重點。**

兩種情況都實測過：

| 浮動斷言的位置 | 結果 |
| --- | --- |
| **後面還有被 `await` 的斷言**（例如放在三行的第一行） | **紅** —— Jest 抓到並正確歸咎於這個 `it` |
| **它是 `it` 的最後一行** | **綠（17 passed）** —— 假綠 |

機制是這樣：`.resolves.toBe()` **回傳一個 Promise**，沒有 `await` 就沒人等它。但 Jest 30 會追蹤未處理的 rejection —— **只要 `it` 還沒結束，它就有機會被抓到並算成失敗**。

所以後面那兩行 `await` 幫了倒忙也幫了正忙：它們讓測試多活了一小段時間，剛好夠那個 rejection 冒出來。放到最後一行，`it` 立刻結束，rejection 沒有人在等 —— **測試綠，錯誤消失。**

放在第一行時，訊息長這樣（Jest 有正確指出是哪一個 `it`）：

```text
● Surveys (e2e) › DELETE /surveys/:id › 刪除問卷會連帶刪掉題目、回覆與答案（cascade）
    expect(received).resolves.toBe(expected)
    Expected: 999
    Received: 0
```
...
> **一個 bug 會不會現形，可能取決於它旁邊那幾行寫了什麼。**

這種「有時候會抓到、有時候不會」的行為最難查 —— 你今天在第一行寫的浮動斷言是紅的，明天重構把它移到最後，它就默默變綠了，而你不會知道。**不要依賴 Jest 幫你接住，把 `await` 寫對。**

`pnpm lint` 在兩種位置都會發出警告（實測輸出）：

```text
357:7  warning  Promises must be awaited, end with a call to .catch, ...
       @typescript-eslint/no-floating-promises
```

**lint 的警告不全是風格建議** —— 這一條是在告訴你「這段程式碼沒有做你以為它在做的事」，而且它比 Jest 可靠：不管放在哪一行都會叫。
