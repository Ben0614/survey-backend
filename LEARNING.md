...
下一步是 **Ch18（前端部署與端到端驗收）**，接續點在下方「**Ch18 接續點**」。
...
**目前狀態**：`pnpm test:e2e` **182 passed**、`pnpm test` **32 passed**、
`tsc --noEmit` 0 errors、`eslint` 0 problems。**Ch0 ~ Ch17 完成，只剩 Ch18。**
...
與第一次 `pnpm build`。判準是**預設值安不安全** —— 所以後端選大聲失敗、前端選退回預設值。

**Ch17 五個功能頁面** — [`ch17`](docs/chapters/ch17-五個功能頁面.md)
六個交付（輪 ⑤ 拆成 a/b），**沒有一個是 bug 修正** —— API 全部照規格運作，
只是沒有人能拿它做出一個畫面。e2e 148 → 182、unit 21 → 32。

| 輪 | 頁面 | 後端交付 |
| :---: | --- | --- |
| ① | `/surveys` | 列表帶 `_count`，消掉 21 次請求的 N+1 |
| ② | `/surveys/new` | 巢狀 create（原子）、刪除放寬到擁有者、跨欄位驗證 |
| ③ | `/surveys/:id/edit` | `PUT` 整份取代（順序＝陣列位置）、`canPublish` |
| ④ | `/surveys/:id/fill` | 整份必答、單選答案要在選項裡（新開 `responses.rules.ts`） |
| ⑤a | 結果頁「摘要」 | `groupBy` 統計端點，**刻意不分頁** |
| ⑤b | 結果頁「個別回應」 | 列表帶 `answers`（消 N+1）、`order` 參數 |

**這一章對「Ch4 的分頁參數好不好用」的答案**：看你在做哪一種頁面 ——
逐筆瀏覽需要它，統計完全不能用它（分頁的設計目的就是「不給你全部」）。

#### Ch18 的起點

主題是**前端部署與端到端驗收**。後端這一側基本上不必再改，要做的是設定與驗收。

**開工前一定要處理的一件事**：

> ⚠️ **線上的 `CORS_ORIGIN` 目前是 `http://localhost:3000`。**
> 前端部署上去之後，那個值必須換成正式的前端網址（多個用逗號分隔）。
> 沒換的症狀很好認：瀏覽器 console 一片 `Failed to fetch`，而 `curl` 打完全正常 ——
> **CORS 是瀏覽器擋下回應，請求其實已經進到後端也執行了**（見 ch14 / ch16）。
> 後端拿不到 `CORS_ORIGIN` 會**啟動失敗**，那是刻意的；設錯值則不會失敗，只會擋掉瀏覽器。

已經備好、**不必**再處理的：

- 前端的 `apiBase` 走 `NUXT_PUBLIC_API_BASE`，`.env.example` 有完整說明
- `pnpm build` 通過，且整條認證鏈在 production 產物（`pnpm preview`）上驗過
- 前端五個頁面全部完成，列表那一排四個按鈕都有目標頁面

**驗收標準**：在**線上**跑通「註冊 → 建立 → 加題目 → 發布 → 填寫 → 看結果」。
本機已經跑通過整條（Ch17 每一輪都用真的瀏覽器驗過），線上會不會撞到新東西
是 Ch18 要回答的。
