...
**重要脈絡（2026-07-31）：** Ch0 的程式碼是 AI 產生的，進度表的 ✅ 只代表環境可用。
使用者對 NestJS 與 Prisma 7 都還陌生，因此補了一批 `[教學]` 註解當作理解鷹架
（見 `docs/專案速查.md` 的「閱讀動線」）。**確認前一章讀得懂，再進下一章。**

**待辦，依序：**

1. **走一遍閱讀動線** —— 從 `src/main.ts` 開始，照每個檔案檔頭的「下一站」讀完九個檔案。
   驗收：能口述「一個 HTTP 請求從進來到拿到回應，經過哪些檔案、依賴注入在哪一刻發生」。
   哪一站講不出來，就補那一站的註解。
2. **讀 NestJS 官方文件 Overview 前四篇**（First steps / Controllers / Providers / Modules，約一小時）。
   加分項不是前提 —— 內容與第 1 步讀的檔案一一對應，等於同一件事的第二個講法。
   Prisma 不要上網找教學：v7 太新，網路上九成是 v5/v6，會是負收益；用 `.agents/skills/` 的官方技能包。
3. **補關聯式資料庫基礎** —— 主鍵、外鍵、一對多、為什麼拆表。**這一步不要跳**，
