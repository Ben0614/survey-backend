...
**進度：Ch0 ~ Ch18 全部完成（2026-09-07）。**

```
後端  https://survey-backend-0dku.onrender.com    Render 免費方案 + Neon production branch
...
**Ch0 ~ Ch18 全部走完。** 這一節取代原本的「接續點」——
不再有「下一章」，只剩下面那份「留給之後的事」。
...
`enableCors` 上線（放 `setup-app.ts`、**不開 credentials**）；前端 `/login` 頁與整條認證鏈完成。
**第一次有真的瀏覽器打後端。**

**Ch15 串接暴露的 API 設計問題** — [`ch15`](docs/chapters/ch15-串接暴露的API設計問題.md)
三輪：讀取的擁有權、驗證規則寫進契約、統一的欄位級錯誤（`fields` 取代 `details`）。
**三個問題 e2e 都抓不到 —— 要有人真的去用才會撞到。**

**Ch16 環境變數分離** — [`ch16`](docs/chapters/ch16-環境變數分離.md)
CORS 白名單改成從 `CORS_ORIGIN` 來（拿不到就**啟動失敗**）、前端第一次 `pnpm build`。
判準是**預設值安不安全** —— 同一條判準在後端與前端得到相反的答案。

**Ch17 五個功能頁面** — [`ch17`](docs/chapters/ch17-五個功能頁面.md)
六個交付，**沒有一個是 bug 修正** —— API 全部照規格運作，只是沒有人能拿它做出一個畫面。
e2e 148 → 182、unit 21 → 32。「分頁參數好不好用」的答案是：**看你在做哪一種頁面**。

**Ch18 前端部署與端到端驗收** — [`ch18`](docs/chapters/ch18-前端部署與端到端驗收.md)
前端上 Vercel、兩個 origin 接起來、線上跑通整條。
最值錢的一句：**SSR 讓「誰在發請求」有兩個答案，而 CORS 只管其中一個** ——
所以 `CORS_ORIGIN` 設錯時首頁是好的、按鈕才壞。

#### 整個專案留下的幾條判準

散在各章的細節不重複，這裡只收「換一個專案也還成立」的那幾條：

- **兩種錯要兩種偵測器。** e2e 抓行為、typecheck 抓型別、契約 diff 抓形狀 ——
  一種偵測器抓不到另一種的錯，而「全綠」只代表你架的那幾種都沒響（Ch13）。
- **e2e 全綠也抓不到「不好用」。** Ch15 與 Ch17 加起來九個交付，
  沒有一個是行為錯誤 —— 全部是「照規格運作，但沒有人能拿它做出一個畫面」。
  **要有人真的去用才會撞到。**
- **一個數字要不要留，看它是不是論證的一部分。** 是斷言本身就留（`swagger.e2e-spec.ts`
  的「paths 共 N 條」）；只是「某份清單有幾項」就刪掉，改成讓清單自己說話 ——
  維護一個會過期的數字，八次裡錯了八次。
- **註解過期八次，而八次都是「當時正確、後來被自己的改動推翻」。**
  有效的對策不是「寫一句提醒」（試過，沒用），是**實測推翻既有說法時立刻全域搜一次那個說法**。
- **規則寫成純函式、放在它所屬的概念裡。** 判準不是「規則都放同一個檔案」——
  Ch17 輪 ④ 因此開了第二個 rules 檔案。而「拿掉外部依賴之後還剩下判斷邏輯」
  是決定要不要寫單元測試的唯一判準。
- **前端每複製一條後端規則，就多一個會安靜過期的地方。**
  型別能從契約產，權限規則不能 —— `tsc` 與契約 diff 都不會叫（Ch17 輪 ①→② 踩過）。

#### 留給之後的事（都不急，但別忘了）

1. **`GET /surveys` 的第四種分類「我填過的」還沒做** —— 要 join `Response`，是新功能不是修 bug。
   它跟下面第 5 條綁在一起：不知道是誰填的，就沒有「我填過的」可以查。
...
3. **`common/field-errors.ts` 的兩個純函式沒有單元測試。** e2e 蓋到了主要路徑
   ...
   **Ch16 的 `cors-origins.spec.ts` 與 Ch17 的 `responses.rules.spec.ts` 是同一條判準的
   第二、第三個現場**，照著它們寫即可。
4. ⚠️ **`role` 有兩個來源，而它們可以不一致**（Ch17 輪 ① 造測試資料時撞到）：

   ```
   RolesGuard        讀 JWT payload 的 role   ← 簽發那一刻的快照（jwt-auth.guard.ts）
   GET /auth/me      查資料庫                  ← 現在的值（auth.service.ts）
...
5. ⚠️ **填答是匿名的，而「要登入才能填、卻不記錄是誰填的」本身就是矛盾**
   （Ch5 的決定，Ch17 輪 ④ 做填寫頁時決定**先不處理**）。連帶三件事做不到：

   ```
   同一個人可以送一百次        擋不住
   「我填過的」分類            做不到（同上面第 1 條）
   填完回到列表顯示「已填過」  做不到
   ```

   它不是加一個檢查就好：要 `Response.userId` + migration + 「一人一次」的唯一鍵，
   而且會牽動「看結果」那幾支的授權（填答者能不能看自己那一筆？）。
   **值得獨立一輪**，混進填寫頁會變成兩個主題。
6. **送出的作答不能修改，也沒有端點可以改**（Ch17 輪 ④ 確認）。
   填寫頁寫著「送出之後無法修改」——那是**目前的事實**，不是一條規則。
   真要做的話會跟第 5 條綁在一起。
7. **摘要會丟掉「不在 `options` 裡的舊答案」**（Ch17 輪 ⑤a）。那是以 `options` 為基準
   組裝的必然結果，而且是刻意的 —— 但它讓 `answerCount` 與各選項 `count` 的總和
   可能對不上。要不要讓那些值以「其他」出現，是產品決定。
8. **舊資料裡有「0 題卻已發布」的問卷**。`canPublish`（Ch17 輪 ③）只擋新的發布，
   不會回頭改既有狀態 —— 填寫頁對它們顯示「這份問卷沒有任何題目」。
   要不要寫一次性的資料清理，是產品決定。
9. **Vercel 的 preview 部署會被 CORS 擋掉**（Ch18 決定不處理）。preview 的網址每次不同
...
**學習紀錄全部留在後端**：`LEARNING.md` 與 `docs/chapters/ch00`～`ch18` 都在
...
（**20 支端點分布在 13 條路徑上**（Ch17 之後）—— `swagger.e2e-spec.ts` 斷言的
是**路徑**數，同一條路徑可以有 GET / PUT / PATCH / DELETE 好幾支。兩個數字都對，別搞混。
Ch17 輪 ③ 的 `PUT /surveys/:surveyId/questions` 只讓「支」數 +1、「條」數不變，
就是因為它加在既有路徑上；輪 ⑤a 的 summary 才是真的多一條路徑。）
...
> **這六章的前端程式碼由 AI 寫，使用者不學前端**（他本來就是前端工程師）。
...
| Ch17 | **五個功能頁面** | 六個交付（輪 ⑤ 拆成 a/b），**沒有一個是 bug 修正** —— API 全部照規格運作，只是沒有人能拿它做出一個畫面。消掉兩個 N+1、讓建立與編輯變成原子的、`order` 從改不了變成陣列位置、補上三條規則（`canPublish` / 整份必答 / 單選答案要在選項裡）、加一支不分頁的統計端點。**「分頁參數好不好用」的答案是：看你在做哪一種頁面** | ✅ |
| Ch18 | **前端部署與端到端驗收** | 前端上 Vercel、`CORS_ORIGIN` 換成正式網址、線上跑通整條。**SSR 讓「誰在發請求」有兩個答案，而 CORS 只管其中一個** —— 所以設錯時首頁是好的、按鈕才壞。順帶修掉使用者回報的兩個前端 bug（登入 401 被當成票過期、搜尋框的 clearable 給 null） | ✅ |

### 貫穿全程的商業規則

規則全部寫成純函式、分散在對應章節實作。**放哪一個檔案的判準是「這條規則屬於
哪個概念」**，不是「規則都放同一份」（Ch17 輪 ④ 因此新開了第二個 rules 檔案）。
**這裡刻意不寫「共 N 條」** —— 底下這兩份清單本身就是答案，
而 `survey.rules.ts` 的檔頭就是為了維護一個會過期的數字而錯了兩次。

`src/surveys/survey.rules.ts` —— 問卷。由 `Survey.status` 決定「**這件事現在能不能做**」
（違反 → 409）：
...
由 `Survey.ownerId` 決定「**你能不能碰**」（違反 → 403）與「**你看不看得到**」
（違反 → 404）：

- **擁有者本人或 `ADMIN` 才能管這份問卷**（Ch12）
- **看不到就當它不存在**（Ch15）—— 回 403 等於承認「這個 id 存在」

`src/responses/responses.rules.ts` —— 作答（Ch17 輪 ④）：

- **這份問卷的題目要全部答完**
- **單選題的答案必須是選項之一**

**順序是「看得到 → 能不能碰 → 現在能不能做」** —— 反過來的話，
不相干的人會拿到「問卷已發布，無法修改題目」這種他不該知道的資訊。
...
**2026-09-06（第四次修訂）** — 把「做五個頁面」從 Ch17 拆出來，階段三從 5 章變 6 章：

原本 Ch17 寫的是「前端部署與端到端驗收」，驗收標準是「跑通建立 → 發布 → 填寫 → 看結果」。
但那條流程**需要五個還沒做的頁面** —— Ch14 只完成了 `/login`，`/surveys` 現在還是佔位頁。
所以那一章實際上混了兩件事：**做五個頁面（大）** 與 **部署 + 端到端（中）**。

拆開的理由就是 `CLAUDE.md` 那條「**一輪只做一個能獨立驗收的主題**」——
五個頁面各自會暴露不同的 API 問題（`/edit` 會撞擁有權 403 與 `DRAFT` 的 409、
`/result` 會撞分頁參數好不好用），混成一章的話，紅燈時分不出是頁面寫錯還是 API 設計有問題。

Ch15 的經驗支持這個判斷：三個已知問題就花了三輪，而課綱自己寫著
「其餘由串接過程發現」—— 那些「其餘」會在做頁面時一次冒出來。
...
| Ch17 — 五個功能頁面 | [`docs/chapters/ch17-五個功能頁面.md`](docs/chapters/ch17-五個功能頁面.md) |
| Ch18 — 前端部署與端到端驗收 | [`docs/chapters/ch18-前端部署與端到端驗收.md`](docs/chapters/ch18-前端部署與端到端驗收.md) |
