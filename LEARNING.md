...
5. ~~**`options: ["", ""]` 建得出「有兩個空白選項」的單選題**~~
   —— **Ch17 輪 ③ 補了 `@IsNotEmpty({ each: true })`，已解決。**

6. ⚠️ **填答是匿名的，而「要登入才能填、卻不記錄是誰填的」本身就是矛盾**
...
7. **送出的作答不能修改，也沒有端點可以改**（Ch17 輪 ④ 確認）。
   填寫頁寫著「送出之後無法修改」——那是**目前的事實**，不是一條規則。
   真要做的話會跟第 6 條綁在一起（不知道是誰填的，就沒有「你的那一筆」可以改）。
